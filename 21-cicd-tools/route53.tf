...
resource "aws_route53_record" "jenkins_agent" {
  ...
  type    = "A"
  ttl     = "1"
  records = [aws_instance.jenkins_agent[0].private_ip] # need to give public ip of instance, lets say we have created jenkins instance, for jenkins we are creating route, so need to public ip , so domain name will give trafic to public
  allow_overwrite = true
}
